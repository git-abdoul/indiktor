...
public class IkrCalypsoEventListener  
extends Observable 
implements PSSubscriber, ConnectionListener {

	private static final Logger LOG = Logger.getLogger(IkrCalypsoEventListener.class);
...
//	protected Map<String, List<PSEvent>> eventFlows = Collections.synchronizedMap(new HashMap<String, List<PSEvent>>());
	...
	private Map<String, List<CalypsoListener>> eventSubscribers;	
	
	public IkrCalypsoEventListener(DSConnection dsConnection, String applicationName) {
		super();
		this.dsConnection = dsConnection;
		this.applicationName = applicationName;
		if (this.dsConnection != null)
			this.dsConnection.setAutoReconnect(true);
		eventSubscribers = new HashMap<String, List<CalypsoListener>>();
	}
	...
	public void subscribeToEvents(CalypsoListener listener, List<String> events) {
		for (String evt : events) {
			List<CalypsoListener> listeners = eventSubscribers.get(evt);
			if (listeners==null) {
				listeners = new ArrayList<CalypsoListener>();
				eventSubscribers.put(evt, listeners);
			}
	...
	public void unsubscribeToEvents(CalypsoListener listener) {
		for (List<CalypsoListener> listeners : eventSubscribers.values()) {
...
//	}
	...
	private void subscribeEvents() throws PSException {
...
//		}
    	...
    	List<CalypsoListener> evtListeners = eventSubscribers.get(event.getClass().getName());
    	if (evtListeners!=null&&evtListeners.size()>0) {
    		for (CalypsoListener listener : evtListeners) {
...
//    }
//    
    public void sendEngineCacheStatsRequest(String engineName) {
    	if (psConnection != null) {
    		try {
        		PSEventEngineRequest ad = new PSEventEngineRequest();
                ad.setType(PSEventEngineRequest.REQUEST_CACHE_STATS);
