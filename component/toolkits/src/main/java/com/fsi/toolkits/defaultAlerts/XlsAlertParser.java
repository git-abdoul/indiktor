...
public class XlsAlertParser {
	...
	public XlsAlertParser(String xlsFilename) {
		alertDefinitions = new HashMap<String, List<AlertDefaultDefinition>>();
		init();
	}

	public void parse(String xlsFilename) {	
		InputStream stream = null;
		try {
			stream = new FileInputStream(new File(xlsFilename));
	...
	private void init() {
		AlertPM alertPM = (AlertPM)ToolkitContext.getBean(PersistencyBeanName.alertPM);
		initGroups(alertPM);
		initDomains(alertPM);
