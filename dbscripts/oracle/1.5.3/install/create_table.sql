...
CREATE TABLE IKR_CATEGORY (
	...
	THRESHOLD FLOAT(24) default '0',
	PERSISTENT NUMBER(1) default 1,
	ARCHIVE NUMBER(1) default 1,
...
CREATE TABLE JOB_SCHEDULER (
		...
		LOGICAL_ENV_ID NUMBER(10) NOT NULL,
		JOB_SCHEDULER_STATIC_DOMAIN_ID NUMBER(20) NOT NULL,
...
CREATE TABLE JOB_SCHEDULER_ATTRIBUTE (
		ID NUMBER(10) NOT NULL PRIMARY KEY,
		JOB_SCHEDULER_ID NUMBER(20) NOT NULL,
		NAME VARCHAR(200) NOT NULL,
		ATTR_VALUE VARCHAR(200) default NULL
) TABLESPACE INDIKTOR;

CREATE SEQUENCE JOB_SCHEDULER_ATTR_ID_SEQ START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER JOB_SCHEDULER_ATTR_TRIGGER
BEFORE INSERT ON JOB_SCHEDULER_ATTRIBUTE
FOR EACH ROW
BEGIN
    SELECT JOB_SCHEDULER_ATTR_ID_SEQ.NEXTVAL INTO :NEW.ID FROM dual;
END;
/
...
CREATE SEQUENCE JOB_SCHEDULER_ST_ID_SEQ start with 1 increment by 1;

CREATE OR REPLACE TRIGGER JOB_SCHEDULER_ST_TRIGGER
BEFORE INSERT ON JOB_SCHEDULER_STATIC_DOMAIN
for each row
begin
    select JOB_SCHEDULER_ST_ID_SEQ.nextval into :new.id from dual;
end;
/
...
CREATE TABLE JOB_SCHEDULER_ATTRIBUTE_CONFIG (
		...
		NAME VARCHAR(100) NOT NULL,
		LABEL VARCHAR(100) NOT NULL,
		SELECTION NUMBER(1) default 0,
		ENABLE NUMBER(1) default 0,
		SELECTION_VALUE VARCHAR(100) default NULL		
)  TABLESPACE INDIKTOR;

CREATE SEQUENCE JOB_SCHEDULER_ATTR_CFG_ID_SEQ START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER JOB_SCHEDULER_ATTR_CFG_TRIGGER
BEFORE INSERT ON JOB_SCHEDULER_ATTRIBUTE_CONFIG
FOR EACH ROW
BEGIN
    SELECT JOB_SCHEDULER_ATTR_CFG_ID_SEQ.NEXTVAL INTO :NEW.ID FROM dual;
END;
/

CREATE TABLE MONITOR_SCHEDULER_TIME (
		MONITOR_SCHEDULER_ID NUMBER(20) NOT NULL,
   ...
   CONSTRAINT PK_MONITOR_SCHEDULER_TIME PRIMARY KEY (MONITOR_SCHEDULER_ID, SCHED_TIME_TYPE)
) TABLESPACE INDIKTOR;
...
CREATE TABLE ALERT_EVENT_COMMENT (
		ALERT_EVENT_ID NUMBER(20) NOT NULL,
		COMMENTS CLOB,
		USER_ID NUMBER(20) NOT NULL
) TABLESPACE INDIKTOR;
...
ALTER TABLE JOB_SCHEDULER_ATTRIBUTE ADD CONSTRAINT FK_JOB_SCHEDULER_ATTRIBUTE FOREIGN KEY (JOB_SCHEDULER_ID) REFERENCES JOB_SCHEDULER (ID) ON DELETE CASCADE;
